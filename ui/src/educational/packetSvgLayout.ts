...
export const SIZES = {
  compact: { bitW: 6, rowH: 12, font: 7.5, titleH: 11, layerGap: 3, stripH: 13 },
  full: { bitW: 10.5, rowH: 26, font: 10, titleH: 16, layerGap: 6, stripH: 20 },
...
export interface LayoutOptions {
  detail: "compact" | "full"; // compact collapses untouched layers to strips
  ruler?: boolean;
}
...
export function layoutPacket(
  ...
  if (diff && diff.removedLayers.length) {
    ...
    for (const removed of diff.removedLayers) {
      ...
      const g = layoutLayer(removed, size, opts.detail === "full" ? "grid" : "strip", gy);
      ghosts.push(g);
    }
  }
